using Commerzbank.NET.Docs.Generator;

if (args.Length == 0)
{
    Console.Error.WriteLine("Usage: Commerzbank.NET.Docs.Generator <wwwroot-path> [--base-url <url>]");
    return 1;
}
...
var baseUrl = "https://emuuu.github.io/Commerzbank.NET";
...
var apiSlugs = apiDocs.Types.Select(t => t.Name).Concat(apiDocs.Enums.Select(e => e.Name)).ToList();
